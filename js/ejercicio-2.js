class Persona {
  ...
  mostrarGeneracion() {
    const generaciones = [
      ...
      {
        nombre: "Generación Millennials Y",
        marcoTemporal: [1981, 1993],
        poblacion: 7200000,
        circunstanciaHistorica: "Inicio de la digitalización",
      ...
      },
      {
        ...
        poblacion: 12200000,
        circunstanciaHistorica: "Paz y explosión demográfica",
    ...
    const generacionActual = generaciones.find(
      (generacion) =>
        this.añoNacimiento >= generacion.marcoTemporal[0] &&
        this.añoNacimiento <= generacion.marcoTemporal[1]
    );

    if (generacionActual) {
      alert(
        `Pertenece a la ${generacionActual.nombre}. Rasgo característico: ${generacionActual.rasgoCaracteristico}`
      );
    } else {
...
function esMayorDeEdad() {
  if (window.persona) {
    const mensaje = window.persona.esMayorDeEdad()
      ? "Es mayor de edad."
      : "No es mayor de edad.";
    alert(mensaje);
  } else {
